...
public class FacadeImplementation extends UnicastRemoteObject implements ApplicationFacadeInterface {
    ...
    public Vector<RuralHouse> getRuralHouses(Owner owner) throws RemoteException,
            Exception {
        return dB4oManager.getRuralHouses(owner);
    }

    public void removeRuralHouse(RuralHouse ruralHouse) throws RemoteException,
            Exception {
        ruralHouse.getOwner().removeRuralHouse(ruralHouse);
        dB4oManager.updateOwner(ruralHouse.getOwner());

        Vector<Offer> offers = ruralHouse.getAllOffers();
        ruralHouse.removeAllOffers();
        dB4oManager.updateRuralHouse(ruralHouse);

        for (Offer o: offers) {
            dB4oManager.removeOffer(o);
        }

        dB4oManager.removeRuralHouse(ruralHouse);
    }
}
