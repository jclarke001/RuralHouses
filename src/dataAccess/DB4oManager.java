...
public class DB4oManager {
    ...
    public void updateOwner(Owner owner) throws RemoteException, Exception {
        if (c.isDatabaseLocal()==false) openObjectContainer();
        try {
            db.store(owner);
            db.commit();
    ...
    public void updateRuralHouse(RuralHouse ruralHouse) throws RemoteException,
            Exception {
    ...
    Exception {
        ...
        if (c.isDatabaseLocal()==false) openObjectContainer();
         ...
         try {
             RuralHouse proto = new RuralHouse(0,owner,null,null);
             ObjectSet result = db.queryByExample(proto);
             Vector<RuralHouse> ruralHouses=new Vector<RuralHouse>();
             while(result.hasNext())
                 ruralHouses.add((RuralHouse)result.next());
    ...
    public void removeRuralHouse(RuralHouse ruralHouse) throws RemoteException, Exception {
        if (c.isDatabaseLocal()==false) openObjectContainer();

        try {
            db.delete(ruralHouse);
            db.commit();
        } finally {
            //db.close();
        }
